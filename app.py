...
@app.route('/admin/rutina/<int:id_rutina>/eliminar', methods=['DELETE'])
@login_required
...
def eliminar_rutina(id_rutina):
    ...
    try:
        ...
        if not rutina:
            return jsonify({'ok': False, 'mensaje': 'Rutina no encontrada.'}), 404

        cur.execute("DELETE FROM rutinas WHERE id_rutina = %s", (id_rutina,))
        mysql.connection.commit()

        return jsonify({'ok': True, 'mensaje': 'Rutina eliminada correctamente.'}), 200

    except Exception as e:
        mysql.connection.rollback()
        return jsonify({'ok': False, 'mensaje': f'Error al eliminar la rutina: {str(e)}'}), 500
...
def rutina():
    catalogo = {
        ...
        "Abdomen": ["Plancha", "Rueda abdominal"]
    }
    ...
    return render_template('semanal.html', catalogo=catalogo, semanal=semanal, orden_dias=["Lunes", "Martes", "Miércoles", "Jueves", "Viernes", "Sábado", "Domingo"])
