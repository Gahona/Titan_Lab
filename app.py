...
suplementos = [
    {"nombre": "Proteína", "imagen": "proteina.jpg", "alt": "Proteína whey", "descripcion": "Aumenta tu recuperación muscular y acelera tus resultados con proteína de alta calidad."},
    {"nombre": "Creatina", "imagen": "creatina.jpg", "alt": "Creatina", "descripcion": "Mejora tu fuerza, potencia y rendimiento en cada entrenamiento."},
    {"nombre": "Pre-entreno", "imagen": "prewornk.jpg", "alt": "Pre-entreno", "descripcion": "Activa tu energía y concentración antes de entrenar."},
    {"nombre": "BCAAs / EAAs", "imagen": "intrawork.jpg", "alt": "BCAAs / EAAs", "descripcion": "Protege tu masa muscular y mejora la recuperación durante y después del entrenamiento."},
    {"nombre": "Omega 3", "imagen": "omega3.jpg", "alt": "Omega 3", "descripcion": "Cuida tu corazón, articulaciones y bienestar general con ácidos grasos esenciales."},
    {"nombre": "Zinc / Magnesio", "imagen": "zmzinc.jpg", "alt": "Zinc y Magnesio", "descripcion": "Mejora tu descanso, recuperación muscular y niveles de energía."},
]
...
def registro():
    if request.method == 'POST':
        ...
        try:
            ...
            cur.execute(
                """INSERT INTO usuarios (nombre, email, password_hash, rol, edad, sexo, altura_cm, peso_kg, objetivo_principal)
...
def login():
    ...
    if request.method == 'POST':
        ...
        cur = mysql.connection.cursor()
        cur.execute("SELECT id_usuario, nombre, password_hash, rol FROM usuarios WHERE email = %s", (email,))
...
def panel_admin():
    ...
    cur.execute("""SELECT r.id_rutina, r.titulo, r.descripcion, r.fecha_creacion, u.nombre AS nombre_usuario
                   FROM rutinas r JOIN usuarios u ON r.id_usuario = u.id_usuario
                   WHERE r.estado = 'pendiente' ORDER BY r.fecha_creacion DESC""")
    rutinas_pendientes = cur.fetchall()
    cur.execute("""SELECT c.id_comentario, c.comentario, c.fecha, u.nombre AS nombre_usuario
                   FROM comentarios_ejercicio c JOIN usuarios u ON c.id_usuario = u.id_usuario
                   WHERE c.estado = 'pendiente' ORDER BY c.fecha DESC""")
    comentarios_pendientes = cur.fetchall()
    cur.execute("""SELECT rp.id_resena, rp.comentario, rp.fecha, u.nombre AS nombre_usuario, 'producto' AS tipo
                   FROM resenas_producto rp JOIN usuarios u ON rp.id_usuario = u.id_usuario
                   WHERE rp.estado = 'pendiente' ORDER BY rp.fecha DESC""")
    resenas_producto = cur.fetchall()
    cur.execute("""SELECT rr.id_resena_rutina AS id_resena, rr.comentario, rr.fecha, u.nombre AS nombre_usuario, 'rutina' AS tipo
                   FROM resenas_rutina rr JOIN usuarios u ON rr.id_usuario = u.id_usuario
                   WHERE rr.estado = 'pendiente' ORDER BY rr.fecha DESC""")
    resenas_rutina = cur.fetchall()
    cur.close()
    return render_template('dashboard_admin.html', total_usuarios=total_usuarios, total_admins=total_admins,
                           total_clientes=total_clientes, total_coaches=total_coaches,
                           rutinas_pendientes=rutinas_pendientes, comentarios_pendientes=comentarios_pendientes,
...
@app.route("/")
def index():
    return render_template("index.html", stats=stats, suplementos=suplementos, colaboradores=colaboradores)
...
def pago_sub(plan_id):
    ...
    return render_template('pago_sub.html', plan_id=plan_id, plan_nombre=plan["nombre"], precio=plan["precio"])
...
def procesar_paypal():
    ...
    if not plan:
        return redirect(url_for('planes'))
    return render_template('pago_exitoso.html', plan_nombre=plan['nombre'], precio=plan['precio'])
...
@app.route('/rutina')
def rutina():
    catalogo = {"Pecho": ["Press banca", "Press inclinado", "Aperturas"], "Espalda": ["Dominadas", "Remo con barra"],
                "Hombro": ["Press militar", "Elevaciones laterales", "Pájaros"], "Pierna": ["Sentadilla", "Prensa", "Extensiones"],
                "Bíceps": ["Curl con barra", "Curl martillo"], "Tríceps": ["Fondos en polea", "Press francés", "Extensión overhead"],
...
def guardar_rutina():
    ...
    total_ejercicios = sum(len(ejercicios) for dia, ejercicios in rutina_json.items() if isinstance(ejercicios, list))
    if total_ejercicios == 0:
        return jsonify({'ok': False, 'mensaje': 'La rutina debe tener al menos un ejercicio.'}), 400
    dias_validos = {"Lunes": "lunes", "Martes": "martes", "Miércoles": "miercoles", "Miercoles": "miercoles",
                    "Jueves": "jueves", "Viernes": "viernes", "Sábado": "sabado", "Sabado": "sabado", "Domingo": "domingo"}
    cur = None
    try:
        cur = mysql.connection.cursor()
        cur.execute("""INSERT INTO rutinas (id_usuario, titulo, descripcion, nivel, publica, estado)
                       VALUES (%s, %s, %s, %s, %s, %s)""",
        ...
        for dia_front, ejercicios in rutina_json.items():
            if dia_front not in dias_validos or not isinstance(ejercicios, list):
                continue
            ...
            for ej in ejercicios:
                ...
                if not fila_ej:
                    continue
                cur.execute("""INSERT INTO rutina_detalle (id_rutina, dia_semana, id_ejercicio, orden_ejercicio,
                               series, repeticiones, descanso_seg, notas)
...
def ver_rutina_admin(id_rutina):
    cur = mysql.connection.cursor()
    cur.execute("""SELECT r.id_rutina, r.titulo, r.descripcion, r.nivel, r.publica, r.estado,
                          r.fecha_creacion, u.nombre AS nombre_usuario
                   FROM rutinas r JOIN usuarios u ON r.id_usuario = u.id_usuario WHERE r.id_rutina = %s""",
                (id_rutina,))
    rutina_row = cur.fetchone()
    if not rutina_row:
        cur.close()
        flash('Rutina no encontrada.', 'error')
        return redirect(url_for('panel_admin'))
    cur.execute("""SELECT rd.id_detalle, rd.dia_semana, rd.orden_ejercicio, rd.series, rd.repeticiones,
                          rd.descanso_seg, rd.notas, e.nombre AS nombre_ejercicio, e.grupo_muscular
                   FROM rutina_detalle rd JOIN ejercicios e ON rd.id_ejercicio = e.id_ejercicio
                   WHERE rd.id_rutina = %s ORDER BY FIELD(rd.dia_semana, 'lunes', 'martes', 'miercoles', 'jueves', 'viernes', 'sabado', 'domingo'), rd.orden_ejercicio ASC""",
                (id_rutina,))
    detalles = cur.fetchall()
    cur.close()
    rutina_por_dias = {'lunes': [], 'martes': [], 'miercoles': [], 'jueves': [], 'viernes': [], 'sabado': [], 'domingo': []}
    for detalle in detalles:
        dia = detalle['dia_semana']
        if dia in rutina_por_dias:
            rutina_por_dias[dia].append(detalle)
    return render_template('detalle_rutina_admin.html', rutina=rutina_row, rutina_por_dias=rutina_por_dias)
